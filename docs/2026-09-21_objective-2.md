...
![](../screenshots/2026-09-20_gold-image-wim-creation-verification.png)
> *Calling up WinPE using `Shift + F10` in the Windows installation prompt, I called
Deployment Image Servicing and Management (dism) to capture the C:\ drive to a WIM 
file on the separate capture disk (D:\\) using maximum compression `/compress:max`. I 
then verified the capture completed by calling `dism /get-imageinfo`. I shut the VM 
down from the command prompt `wpeutil shutdown` and detached the capture disk for 
import into IMG-SRV-01.*